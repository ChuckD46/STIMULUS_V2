...
namespace STIMULUS_V2.Server.Services
{
    public class NoeudEtudiantService : INoeudEtudiantService
    {
        ...
        public async Task<APIResponse<Noeud_Etudiant>> GetProgression(string da, int graphe, Noeud noeud)
        {
            try
            {
                ...
                var ChildrenNode = await sTIMULUSContext.Noeud.Where(item => item.NoeudParentId == noeud.NoeudId).ToListAsync();
                var ParentNode = await GetByNoeudAndDa(noeud.NoeudId, da);
                ...
                foreach (var childNode in ChildrenNode)
                {
                    ...
                    if (Graphe.Status == GrapheStatus.DESACTIVER)
                    {
                        ...
                        {
                            if (ChildNode.Data.Status == 1)
                            {
                                ChildNode.Data.Status = 1;
                            }
                            else
                ...
                return new APIResponse<Noeud_Etudiant>(ParentNode.Data, 200, "Succès");
            }
            catch (Exception ex)
            {
                return new APIResponse<Noeud_Etudiant>(null, 500, "Erreur dans la progression");
            }
        }
